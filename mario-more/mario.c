...
int main(void)
{
    ...
    for (int i = height; i >= 1; i--) //for rows
    {
        ...
        for (int l = 1; l <= 2; l++) //iterating two times for two pyramids
        {
            for (int k = i; k <= height; k++) //for hashes both(left and right pyrmaid)
            {
                printf("#");
            }
            if (l != 2) //avoiding printing extra spaces at last
            {
                printf("  ");
            }
        }
